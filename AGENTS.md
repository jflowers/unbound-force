...
| CRAP Load | `ci_crapload.yml` | PR to main | CRAP/GazeCRAP regression analysis + epsilon tolerance (ε=0.5) + PR comment |
| CRAP Baseline | `ci_baseline_update.yml` | push to main | Auto-regenerate `.gaze/baseline.json` on CI runner for cross-platform parity |
...
- 036-ci-crap-baseline: Added CI-generated CRAP baseline and epsilon tolerance for cross-platform score stability. Created `ci_baseline_update.yml` workflow that regenerates `.gaze/baseline.json` on the CI runner (Ubuntu x86_64) after each merge to main, eliminating false CRAP regressions caused by cross-platform coverage differences between macOS ARM (local) and Ubuntu x86_64 (CI). Added `evaluate` job to `ci_crapload.yml` with epsilon tolerance (ε=0.5) — CRAP/GazeCRAP deltas at or below 0.5 are treated as noise. Post-PR comment now includes epsilon filter summary. Bot actor guard prevents recursive CI triggers. Concurrency group prevents simultaneous baseline updates. Filed upstream issue `complytime/org-infra#223` proposing `regression-epsilon` input for the reusable workflow. No Go code changes — CI workflow YAML + documentation only. 2 user stories and 14 tasks completed.
